...
class CartPoleActorCritic(tf.keras.Model):
    def __init__(self, num_actions: int, num_hidden_units: int, seed: int = 42):
        """Initialize."""
        super().__init__()
    ...
    def _run_episode(self, max_steps: int) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        initial_state: tf.Tensor = tf.constant(
            self.env.reset(), dtype=tf.float32)
        ...
        for t in tf.range(max_steps):
            ...
            action_logits_t, value = self._forward(state)
    ...
    def render_episode(self, max_steps: int):
